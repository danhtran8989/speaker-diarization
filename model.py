...
# Detect available providers once
...
def get_speaker_diarization(
    ...
    # Critical fix: use -1 instead of None for automatic speaker detection
    clusters_to_use = num_clusters if num_clusters > 0 else -1
    ...
    config = sherpa_onnx.OfflineSpeakerDiarizationConfig(
        ...
        clustering=sherpa_onnx.FastClusteringConfig(
            num_clusters=clusters_to_use,   # ← Fixed: -1 for auto
            threshold=threshold,
        ),
        min_duration_on=0.3,
    ...
    if not config.validate():
        raise RuntimeError("Config validation failed – check if all model files exist.")
