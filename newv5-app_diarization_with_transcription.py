#!/usr/bin/env python3
# newv5-app_diarization_with_transcription.py
"""
...
def process_audio_with_transcription(
    ...
    try:
        clustering_threshold = float(input_threshold) if num_speakers <= 0 else 0.5  # default if not used
    except ValueError:
        return "", build_result_html("", "Ngưỡng clustering không hợp lệ.", "")

    start_time = time.time()

    # Key fix: pass -1 for auto speaker count, not None
    sd = get_speaker_diarization(
        segmentation_model=speaker_segmentation_model,
        embedding_model=embedding_model,
        num_clusters=num_speakers if num_speakers > 0 else -1,   # ← Fixed here
        threshold=clustering_threshold,
    )

    segments = sd.process(audio).sort_by_start_time()

    # Convert full audio to pydub once
